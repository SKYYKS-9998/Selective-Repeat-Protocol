...
PUBLIC	??_C@_0M@OEFLMFMI@Apr?530?52022@		; `string'
...
CONST	ENDS
;	COMDAT ??_C@_0M@OEFLMFMI@Apr?530?52022@
CONST	SEGMENT
??_C@_0M@OEFLMFMI@Apr?530?52022@ DB 'Apr 30 2022', 00H	; `string'
CONST	ENDS
...
; File D:\Cproject\NETLab1-Windows-VS2019exp\protocol.c
;	COMDAT randB
_TEXT	SEGMENT
randB	PROC						; COMDAT
...
; 625  : {
	...
	lea	rcx, OFFSET FLAT:__D8B7321B_protocol@c
	call	__CheckForDebuggerJustMyCode

; 626  :     static unsigned int holdrand = 0x1e459090;
; 627  :     return ((holdrand = holdrand * 214013L + 2531011L) >> 16) & 0x7fff;
...
; 620  :     static unsigned int holdrand = 0x65109bc4;
; 621  :     return ((holdrand = holdrand * 214013L + 2531011L) >> 16) & 0x7fff;
...
; 593  :     static int last_ts = 0;
; 594  : 
; 595  :     if (!network_layer_active)
...
; 600  : 
; 601  :     if ((now - last_ts) * CHAN_BPS / 8 / 1000 < PKT_LEN * 3 / 4)
...
; 605  :         if (now / 1000 / mode_cycle % 2 != mode_ibib) {
...
; 606  :             if (now - last_ts < 4000 + rand() % 500)
...
; 608  :         }
; 609  :         if (now < CHAN_DELAY + 3 * PKT_LEN * 8000 / CHAN_BPS)
...
; 570  :             return i == ACK_TIMER_ID ? ACK_TIMEOUT : DATA_TIMEOUT;
...
; 546  :     if (nr >= ACK_TIMER_ID || timer[nr] == 0)
...
; 548  :     return timer[nr] > now ? timer[nr] - now : 0;
...
; 511  :     struct BLK *blk = rblk_head;

	mov	rax, QWORD PTR rblk_head
	mov	QWORD PTR blk$[rbp], rax

; 512  : 
; 513  :     if (blk == NULL || blk->commit_ts > now) 
...
; 514  :         ABORT("recv_byte(): Receiving Queue is empty");
...
; 465  :     unsigned char *p;
; 466  : 
; 467  :     blk = (struct BLK *)malloc(sizeof(struct BLK));
...
; 472  :     blk->wptr = recv(sock, (char *)blk->data, BLKSIZE, 0);

	mov	rax, QWORD PTR blk$[rbp]
	add	rax, 24
	movsxd	rcx, DWORD PTR sock
	xor	r9d, r9d
...
; 481  :         int a;
; 482  :         double rate, fact;
; 483  : 
; 484  :         rate = (double)noise / nbits;
...
; 486  :         a = (int)((1.0 - pow(1.0 - ber, fact * blk->wptr)) * (RAND_MAX + 1.0) + 0.5);
	...
	movsd	QWORD PTR tv133[rbp], xmm1
	movaps	xmm1, xmm0
...
; 492  :                 dbg_warning("Impose noise on received data, %u/%u=%.1E\n", noise, nbits, (double)noise / nbits);
...
; 495  :     }
; 496  : 
; 497  :     blk->commit_ts = now + CHAN_DELAY - 10;
...
; 420  :     static int last_ts = 0;
; 421  :     int n, send_tail = sq_head, send_bytes;
...
; 428  : 
; 429  :     send_bytes_allowed = (now - last_ts) * CHAN_BPS / 8 / 1000 * 2;
...
; 434  : 
; 435  :     if (send_tail >= sq_head) 

	mov	eax, DWORD PTR sq_head
	cmp	DWORD PTR send_tail$[rbp], eax
	jl	SHORT $LN5@socket_sen

; 436  :         send_bytes = send_sq_data(sq_head, send_tail);
...
; 437  :     else {
; 438  :         send_bytes = send_sq_data(sq_head, SQ_SIZE);
...
; 439  :         send_bytes += send_sq_data(0, send_tail);
...
; 443  :     send_bytes_allowed -= send_bytes;
...
; 408  : 
; 409  :     ret = send(sock, (char *)&sq[start], end1 - start, 0);
	...
	add	rdx, rcx
	mov	rcx, rdx
	movsxd	rdx, DWORD PTR sock
	mov	QWORD PTR tv73[rbp], rdx
	xor	r9d, r9d
	mov	r8d, eax
	mov	rdx, rcx
...
; 374  :     inform_phl_ready = 1;

	mov	DWORD PTR inform_phl_ready, 1

; 375  : 
; 376  :     if (send_bytes_allowed && sq_head == sq_tail) {
...
; 377  :         send(sock, (char *)&byte, 1, 0);

	movsxd	rax, DWORD PTR sock
	xor	r9d, r9d
	mov	r8d, 1
...
; 383  :         ABORT("Physical Layer Sending Queue overflow");
...
; 364  :     return (sq_tail + SQ_SIZE - sq_head) % SQ_SIZE;
...
; 144  : 			"\nOptions : \n"
; 145  : 			"    -?, --help : print this\n"
; 146  : 			"    -u, --utopia : utopia channel (an error-free channel)\n"
; 147  : 			"    -f, --flood : flood traffic\n"
; 148  : 			"    -i, --ibib  : set station B layer 3 sender mode as IDLE-BUSY-IDLE-BUSY-...\n"
; 149  : 			"    -n, --nolog : do not create log file\n"
; 150  : 			"    -d, --debug=<0-7>: debug mask (bit0:event, bit1:frame, bit2:warning)\n"
; 151  : 			"    -p, --port=<port#> : TCP port number (default: %u)\n"
; 152  : 			"    -b, --ber=<ber> : Bit Error Rate (received data only)\n"
; 153  : 			"    -l, --log=<filename> : using assigned file as log file\n"
; 154  : 			"    -t, --ttl=<seconds> : set time-to-live\n"
; 155  : 			"\n"
; 156  : 			"i.e.\n"
; 157  : 			"    %s -fd3 -b 1e-4 A\n"
; 158  : 			"    %s --flood --debug=3 --ber=1e-4 A\n"
; 159  : 			"\n",
; 160  : 			DEFAULT_PORT, argv[0], argv[0]);
; 161  : 		exit(0);
...
; 165  : 
; 166  : 	while ((opt = getopt_long(argc, argv, OPT_SHORT, intopts, NULL)) != -1) {
...
; 190  : 
; 191  : 		case 'p':
; 192  : 			port = (unsigned short)atoi(optarg);
...
; 206  : 
; 207  : 		case 't':
; 208  : 			mode_life = atoi(optarg) * 1000; /* ms */
...
; 210  : 
; 211  : 		default:
; 212  : 			printf("ERROR: Unsupported option\n");
...
; 219  : 
; 220  : 	station = tolower(argv[optind++][0]);
...
; 222  : 		ABORT("Station name must be 'A' or 'B'");
...
; 226  : 		if (stricmp(fname + strlen(fname) - 4, ".exe") == 0)
...
; 228  : 		strcat(fname, station == 'a' ? "-A.log" : "-B.log");
...
; 233  : 	else if ((log_file = fopen(fname, "w")) == NULL) 
...
; 234  : 		printf("WARNING: Failed to create log file \"%s\": %s\n", fname, strerror(errno));
...
; 240  : 		station_name());
; 241  : 
; 242  : 	lprintf("Protocol.lib, version %s, jiangyanjun0718@bupt.edu.cn\n", VERSION, __DATE__);

	lea	r8, OFFSET FLAT:??_C@_0M@OEFLMFMI@Apr?530?52022@
	lea	rdx, OFFSET FLAT:??_C@_03EMGHBCAM@4?40@
	lea	rcx, OFFSET FLAT:??_C@_0DH@PFKHDMOI@Protocol?4lib?0?5version?5?$CFs?0?5jiang@
	call	lprintf

; 243  : 	lprintf("Channel: %d bps, %d ms propagation delay, bit error rate ", CHAN_BPS, CHAN_DELAY);
...
; 248  : 	lprintf("Log file \"%s\", TCP port %d, debug mask 0x%02x\n", fname, port, debug_mask);
...
; 908  : 		if (head_magic[i] != HEAD_MAGIC)
...
; 913  : 		if (foot_magic[i] != FOOT_MAGIC)
...
; 917  : 
; 918  : exit:
; 919  : 	ABORT("Memory used by 'protocol.lib' is corrupted by your program");
...
; 898  :         head_magic[i] = HEAD_MAGIC;

	movsxd	rax, DWORD PTR i$[rbp]
	lea	rcx, OFFSET FLAT:head_magic
	mov	DWORD PTR [rcx+rax*4], -1515854821	; a5a5e41bH

; 899  :         foot_magic[i] = FOOT_MAGIC;;
...
; 116  :     return (char *)(station == 'a' ? "A" : station == 'b' ? "B" : "XXX");
...
; 533  :     if (nr >= ACK_TIMER_ID) 

	cmp	DWORD PTR nr$[rbp], 128			; 00000080H
	jb	SHORT $LN5@start_time
$LN4@start_time:

; 534  :         ABORT("start_timer(): timer No. must be 0~128");
...
; 535  :     timer[nr] = now + phl_sq_len() * 8000 / CHAN_BPS + ms;
...
; 739  :         ABORT("recv_frame(): Receiving Queue is empty");
...
; 744  :         sprintf(msg, "recv_frame(): %d-byte buffer is too small to save %d-byte received frame", size, len);
...
; 746  :     }
; 747  :     
; 748  :     memcpy(buf, rf_head->frame, len);
...
; 657  :     static int last_ts = 0;
; 658  :     int i, (*my_rand)(void) = station == 'a' ? randB : randA;
...
; 665  :             ABORT("Network Layer received a bad packet from data link layer");
...
; 669  : 
; 670  :     if (now - last_ts > 2000 && now > ts0 + 2000) {
...
; 671  :         double bps;
; 672  :         bps = (double)rbytes * 8 * 1000 / (now - ts0);
...
; 673  :         lprintf(".... %d packets received, %.0f bps, %.2f%%, Err %d (%.1e)\n", 
...
; 674  :             rpackets, bps, bps / CHAN_BPS * 100, noise, (double)noise/nbits);
; 675  :         last_ts = now;
...
; 636  :     static int pkt_no = 0;
; 637  :     int i, len;
; 638  :     int (*my_rand)(void) = station == 'a' ? randA : randB;
...
; 641  :         ABORT("get_packet(): Network layer is not ready for a new packet");
...
; 646  :     *(unsigned short *)packet = (station - 'a' + 1) * 10000 + (pkt_no++ % 10000);
...
; 769  :      
; 770  :         /* commit received socket data */
; 771  :         if (rblk_head && rblk_head->commit_ts <= now) {
...
; 772  :             n = rblk_head->wptr - rblk_head->rptr;
...
; 784  :                         rf_buf = (struct RCV_FRAME *)calloc(1, sizeof(struct RCV_FRAME));
...
; 796  :                 } else if (rf_buf && rf_buf->len < sizeof(rf_buf->frame)) {
...
; 797  :                     if (rf_buf->state == 0) {

	mov	rax, QWORD PTR rf_buf
	cmp	DWORD PTR [rax+4], 0
	jne	SHORT $LN28@wait_for_e

; 798  :                         rf_buf->frame[rf_buf->len] = ch;
...
; 801  :                         rf_buf->frame[rf_buf->len] |= (ch << 4) ^ (ch & 0xf0);
...
; 810  :         }
; 811  :         
; 812  :         /* test socket send/receive */
; 813  :         tm.tv_sec = tm.tv_usec = 0;
...
; 818  : 
; 819  :         if (select(sock + 1, &rfd, &wfd, 0, &tm) < 0) 
...
; 829  : 
; 830  :         /* network layer event */
; 831  :         if (network_layer_ready()) {
...
; 833  :             return NETWORK_LAYER_READY;
...
; 834  :         }
; 835  : 
; 836  :         /* check all timers */
; 837  :         if ((event = scan_timer(arg)) != 0)
...
; 839  : 
; 840  :         /* physical layer event */
; 841  :         if (inform_phl_ready && phl_sq_len()  < PHL_SQ_LEVEL) {
...
; 842  :             inform_phl_ready = 0;

	mov	DWORD PTR inform_phl_ready, 0

; 843  :             return PHYSICAL_LAYER_READY;
...
; 854  :             if (t > mode_tick + 50 && time(0) > last_warn + 1) {
...
; 855  :                 lprintf("** WARNING: System too busy, sleep %d ms, but be awakened %d ms later\n", 
...
; 868  :                 ticks = (ms - start_ms) / mode_tick;
...
; 869  :                 lprintf("====== CPU BUSY for %d ms (cnt %d)\n", ms - wakeup_ms, ++busy_cnt);
...
; 870  :                 lprintf("------ noSleep %d, sleep %d, Elapse %d ticks\n", ticks - sleep_cnt, sleep_cnt, ticks);
...
; 879  :             if (ms > mode_tick + 1 || ms < mode_tick - 1) 
...
; 266  : 
; 267  :         name.sin_family = AF_INET;

	mov	eax, 2
	mov	WORD PTR name$[rbp], ax

; 268  :         name.sin_addr.s_addr = INADDR_ANY;

	mov	DWORD PTR name$[rbp+4], 0

; 269  :         name.sin_port = htons(port);
...
; 270  : 
; 271  :         admin_sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);

	mov	r8d, 6
	mov	edx, 1
	mov	ecx, 2
	call	socket
	mov	DWORD PTR admin_sock$[rbp], eax

; 272  :         if (admin_sock < 0) 

	cmp	DWORD PTR admin_sock$[rbp], 0
	jge	SHORT $LN21@protocol_i
$LN4@protocol_i:

; 273  :             ABORT("Create TCP socket");
...
; 274  :         if (bind(admin_sock, (struct sockaddr *)&name, sizeof(name)) < 0) {

	movsxd	rax, DWORD PTR admin_sock$[rbp]
	mov	r8d, 16
...
; 275  :             lprintf("Station A: Failed to bind TCP port %u", port);
...
; 276  :             ABORT("Station A failed to bind TCP port");
...
; 280  : 
; 281  :         lprintf("Station A is waiting for station B on TCP port %u ... ", port);
...
; 283  : 
; 284  :         sock = accept(admin_sock, 0, 0);
...
; 285  :         if (sock < 0) 
	...
	cmp	DWORD PTR sock, 0
	jge	SHORT $LN23@protocol_i
$LN10@protocol_i:

; 286  :             ABORT("Station A failed to communicate with station B");
...
; 288  : 
; 289  :         recv(sock, (char *)&epoch, sizeof(epoch), 0);
...
; 297  :         if (sock < 0) 
	...
	cmp	DWORD PTR sock, 0
	jge	SHORT $LN25@protocol_i
$LN13@protocol_i:

; 298  :             ABORT("Create TCP socket");
...
; 299  : 
; 300  :         name.sin_family = AF_INET;

	mov	eax, 2
	mov	WORD PTR name$[rbp], ax

; 301  :         name.sin_addr.s_addr = inet_addr("127.0.0.1");

	lea	rcx, OFFSET FLAT:??_C@_09PFCHIMEP@127?40?40?41@
	call	inet_addr
	mov	DWORD PTR name$[rbp+4], eax

; 302  :         name.sin_port = htons((short)port);
...
; 305  :             lprintf("Station B is connecting station A (TCP port %u) ... ", port);
...
; 307  : 
; 308  :             if (connect(sock, (struct sockaddr *)&name, sizeof(struct sockaddr_in)) < 0) {
...
; 317  :             ABORT("Station B failed to connect station A");
...
; 320  :         send(sock, (char *)&epoch, sizeof(epoch), 0);
...
; 322  : 
; 323  :     {
; 324  :         struct tm *newtime;
; 325  :         newtime = localtime(&epoch);

	lea	rcx, OFFSET FLAT:epoch
	call	localtime
	mov	QWORD PTR newtime$7[rbp], rax

; 326  :         lprintf("New epoch: %s", asctime(newtime));
...
; 330  :     /* socket options */
; 331  :     {
; 332  :         int timeout_ms = 10; 

	mov	DWORD PTR timeout_ms$8[rbp], 10

; 333  :         int buf_size = 1024 * 64;
...
; 335  : 
; 336  :         setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout_ms, sizeof(int));
...
; 337  :         setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout_ms, sizeof(int));
...
; 338  : 
; 339  :         setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (char *)&buf_size, sizeof(int));
...
; 340  :         setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (char *)&buf_size, sizeof(int));
...
; 341  : 
; 342  :         setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char *)&on, sizeof(on));   
